...
{
	//check sizes
#define logsizeof(x) printf("Size of %s is %d\n",#x,sizeof(x));

	logsizeof(uuid);
	logsizeof(BlockInfo_u);
	logsizeof(StreamFragment);
	logsizeof(DirEntry);
	logsizeof(NodeIndexEntry);
	logsizeof(StreamEntry);
	logsizeof(NodeInfo);
	logsizeof(Header);
	...
	//simulate
	device_handle dev=DevOpen("G:\\fstest.bfs",DEV_READWRITE);
